...
countries <- c("USA", "RUS", "GBR", "CHN", "VNM") 
# ISO country codes for USA, UK, Russia, China
df_countries <- df_temp_change[df_temp_change$code %in% countries,]
...
for (country in unique_countries) {
  ...
  p <- ggplot(country_data, aes(x = Year, y = Value)) +
    ...
    labs(title = paste("Linear Regression of Values Over Years for", unique(country_data$Name)),
         x = "Year",
         y = "Value") +
    theme_minimal()
...
```{r}
# Predict the value for the next year (2023)
new_data <- data.frame(Year = 2023)
prediction <- predict(model, newdata = new_data)
print(paste("Predicted value for the year 2023:", prediction))
```
...
ui <- fluidPage(
  titlePanel("Global Surface Temperature Change with Year Slider"),
  sidebarLayout(
    sidebarPanel(
